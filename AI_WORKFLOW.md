...
**Time budget as a forcing function.** Human reported cumulative time each turn. Claude flagged the ≥ 3h30m threshold and adjusted scope accordingly: E2E specs were written to cover only the critical paths (create hero, full pentathlon run), and T-43/T-44/T-45 (a11y, mobile, Lighthouse) were explicitly deferred to manual human review rather than automated.
...
| 4h20m | +10m | Round 7 (partial) implemented: T-41 (`ErrorBlock.vue` + 4 tests; wired into `HeroesPage` error section, replacing the inline alert block), T-46 (`e2e/heroes-crud.spec.ts` — opens form, fills name + 128×128 PNG fixture, submits, asserts new card + toast, cleans up via API), T-47 (`e2e/pentathlon-run.spec.ts` — seeds 3 heroes via API, selects them, walks 5 events, asserts podium with Oro/Plata/Bronce labels, cleans up). T-42 already covered by `index.html lang="es"` + `router.afterEach` title wiring. Added 128×128 PNG fixture (257 bytes, deflate-encoded RGB) and `e2e/helpers/api.ts` with .env.local parser. 281 unit tests + 3 E2E specs all green; lint + typecheck + build all green. T-43 (a11y audit pass), T-44 (mobile pass), T-45 (Lighthouse perf smoke) require manual browser-based review and are flagged for the user. Notable AI correction: `HeroSelector.handleSimulate()` was calling `pentathlonStore.advance()` immediately after `start()`, which jumped past event 1 (cursor 0→1 = events[1]). E2E pentathlon-run caught it on the 4th "Siguiente prueba" click failing because the user was already on event 5 with the "Ver clasificación" CTA. Removed the spurious advance so `start()` cleanly lands on event 1. |
| 4h30m | +10m | Round 8 (T-48..T-50): `AI_WORKFLOW.md` tools/roles + prompting-strategy sections filled; README status updated to 50/50; `pnpm lint` + `pnpm typecheck` + 281 unit tests + 6 E2E specs (desktop + mobile) all green; no `console.*` in `src/`. Cross-page nav buttons added (`/heroes ↔ /pentatlon` ghost CTAs in PageHeader). Notable AI failures section extended with Round 6 + Round 7 cases. |